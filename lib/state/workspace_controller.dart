...
class WorkspaceController extends ChangeNotifier {
  ...
  WorkspaceController({ProjectModel? initial, CanvasSize? newSize}) {
    project = initial ?? ProjectModel(
      ...
      pages: [DesignPage(title: 'Page 1', size: newSize ?? const CanvasSize(1080, 1080))],
    );
    if (project.pages.isEmpty) {
      project.pages.add(DesignPage(title: 'Page 1', size: newSize ?? const CanvasSize(1080, 1080)));
    }
  }
  ...
  DesignElement addText({String text = 'اپنا متن یہاں لکھیں', bool rtl = true}) {
    _checkpoint();
    final e = DesignElement(id: _newId('text'), kind: ElementKind.text, x: page.size.width * .1, y: page.size.height * .35, width: page.size.width * .8, height: 150, text: text, fontSize: 64, textDirection: rtl ? TextDirection.rtl : TextDirection.ltr);
    page.elements.add(e); selectedId = e.id; _changed(); return e;
  }

  DesignElement addShape() {
    _checkpoint();
    final e = DesignElement(id: _newId('shape'), kind: ElementKind.shape, x: page.size.width * .35, y: page.size.height * .3, width: 320, height: 220, colorValue: const Color(0xFF7C3AED).toARGB32());
    page.elements.add(e); selectedId = e.id; _changed(); return e;
  }

  DesignElement addImage(Uint8List bytes) {
    _checkpoint();
    final e = DesignElement(id: _newId('image'), kind: ElementKind.image, x: page.size.width * .15, y: page.size.height * .2, width: page.size.width * .7, height: page.size.height * .45, imageBytes: bytes);
    page.elements.add(e); selectedId = e.id; _changed(); return e;
  }
  ...
  void centerSelected() {
    final e = selected; if (e == null || e.locked) return; _checkpoint();
    e.x = (page.size.width - e.width) / 2;
    e.y = (page.size.height - e.height) / 2;
    _changed();
  }
  ...
  void resizeSelectedFromHandle(String handle, double dx, double dy) {
    final e = selected; if (e == null || e.locked) return;
    final c = math.cos(e.rotation), s = math.sin(e.rotation);
    final localDx = dx * c + dy * s, localDy = -dx * s + dy * c;
    const minSize = 32.0;
    double newWidth = e.width, newHeight = e.height;
    if (handle.contains('right')) newWidth += localDx;
    if (handle.contains('left')) newWidth -= localDx;
    ...
    final shiftX = handle.contains('left') ? e.width - newWidth : 0.0;
    final shiftY = handle.contains('top') ? e.height - newHeight : 0.0;
    e.x += shiftX * c - shiftY * s;
    e.y += shiftX * s + shiftY * c;
    e.width = newWidth; e.height = newHeight; _changed();
  }
  ...
  void resizeCanvas(double width, double height) {
    if (width < 64 || height < 64) return; _checkpoint(); final old = page.size; final sx = width / old.width, sy = height / old.height; page.size = CanvasSize(width, height);
    for (final e in elements) { e.x *= sx; e.y *= sy; e.width *= sx; e.height *= sy; }
    _changed();
  }
