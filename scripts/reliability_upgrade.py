...
# Manual save should also cancel a pending debounce so it does not immediately
# perform a duplicate write after the explicit save.
text = text.replace("  Future<void> _save() async {\n    try {", "  Future<void> _save() async {\n    _autosaveTimer?.cancel();\n    try {", 1)

path.write_text(text)
print('Crash-recovery autosave layer applied successfully')
